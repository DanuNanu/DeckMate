...
func try_castelling(target_tile:Vector2i, tile_pos: Vector2) -> bool:
	...
	if (is_selected and has_moved == false and occupied[target_tile] != null and occupied[target_tile].colour == self.colour and occupied[target_tile]._get_piece_type() == 1 and not occupied[target_tile].has_moved):
		if is_cleared(target_tile, position_on_grid):
			...
			if (occupied[target_tile].get_pos().x == 0) :
				just_moved = true
				has_moved = true
				rook.has_moved = true
				...
				has_moved = true
				rook.has_moved = true
				position_on_grid = Vector2i(6,7)
				global_position = tilemap.map_to_local(Vector2i(6,7))
				occupied[Vector2i(5,7)] = rook
				rook.position_on_grid = Vector2i(5,7)
				rook.global_setter(tilemap.map_to_local(Vector2i(5,7)))
				occupied[target_tile] = null
				highlight_map.clear()
				print("Castlled black rook with black king")
				return true
