...
func try_move_to(target_tile: Vector2i, tile_pos: Vector2)-> bool:
	...
	if is_selected and (is_diagonal or is_straight):
		if is_cleared(target_tile, position_on_grid):
			...
			print("Moved black Queen to ", position_on_grid, " (world position: ", global_position, ")")
			return true
	print("Invalid move")
	return false
...
func try_take_over(target_tile: Vector2i, tile_pos: Vector2) -> bool:
	...
	if is_selected and (is_diagonal or is_straight):
		if is_cleared(target_tile, position_on_grid):
			...
			position_on_grid = target_tile
			global_position = tile_pos
			print("Black Queen takes over piece at ", position_on_grid, " (world position: ", global_position, ")")
			return true
	print("Invalid move")
	return false
	...
	if is_hovered and event is InputEventMouseButton:
		if not is_selected: 
			...
			if mouse_event.button_index== MOUSE_BUTTON_LEFT and mouse_event.pressed:
				...
				print("black Queen selected at tile: ", position_on_grid)
